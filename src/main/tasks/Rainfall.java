...
public class Rainfall {
        ...
        for (String line : lines) {
            ...
            if (separated[0].equals(town)) {
                ...
                for (int j = 1; j <= separated_two.length; j += 2) {
                    total += Double.parseDouble(separated_two[j]);
                }

            }
        }
        double answer = total / 12;
        if (answer == 0.0) {
            return -1;
        }
        return answer;
    ...
    public static double variance(String town, String strng) {
        ...
        for (Double aDouble : totals) {
            total += Math.pow(aDouble, 2);
        }
        double answer = total / 12;
        if (answer == 0.0) {
            return -1;
    ...
    public static void runTask15(BufferedReader reader) throws IOException {
        try {
            ...
            System.out.println("Average of rainfall for " +city+ " from data: " + mean(city,data));
            System.out.println("Variance of rainfall for " +city+ " from data: " + variance(city,data));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
